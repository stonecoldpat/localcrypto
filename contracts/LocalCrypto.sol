...
contract LocalCrypto {
  ...
  // a - b = c (include your own modulus)
  function submod(uint a, uint b, uint prime) returns (uint){
      uint a_nn;
      ...
      uint c = addmod(a_nn - b,0,prime);

      return c;
  }

  // Create a public key using second genrator
  function createPubKeyWithY(uint r) returns (uint[2] key){
    uint[3] memory jacob_key = Secp256k1_noconflict._mul(r,Y);
    ECCMath_noconflict.toZ1(jacob_key, pp);
  ...
  // Fuction used by test framework to quickly perform multiplication
  function mul(uint r, uint[2] gen) returns (uint[2] res){
    uint[3] memory temp = Secp256k1_noconflict._mul(r, gen);
  ...
  // Fuction used by test framework to quickly perform multiplication
  function mulG(uint r) returns (uint[2] res){
    uint[3] memory temp = Secp256k1_noconflict._mul(r, G);
    ECCMath_noconflict.toZ1(temp, pp);
    res[0] = temp[0];
    res[1] = temp[1];
  }

  function mulY(uint r) returns (uint[2] res){
    uint[3] memory temp = Secp256k1_noconflict._mul(r, Y);
    ECCMath_noconflict.toZ1(temp, pp);
    res[0] = temp[0];
    res[1] = temp[1];
  }

  function getG() returns (uint[2]){
    return G;
  }

  function getY() returns (uint[2]) {
    return Y;
  }

  // Combine three shares that have the form y^{s_{i}} to recreate secret y^{s}.
  function joinShares(uint[2] share1, uint[2] share2, uint[2] share3, uint[3] pos) returns (uint[3] accum){
    uint prime = nn;
    ECPoint[] memory points = new ECPoint[](3);
    points[0] = ECPoint(share1);
    points[1] = ECPoint(share2);
    points[2] = ECPoint(share3);
    ...
    for(uint formula = 0; formula < points.length; formula++) {
      ...
      for(uint count=0; count<points.length; count++) {

        if(formula != count) {
          uint startpos = pos[formula];
          uint nextpos = pos[count];

          // x_{m}, where m is inner loop
          numerator = mulmod(numerator,nextpos,prime);
          // x_{m} - x_{j}, where j is outter loop
          denominator = mulmod(denominator,submod(startpos,nextpos,prime),prime);
        }
      }

      // Numerator / Denominator i.e. x_{m} / x_{m} - x_{j}
      uint ndd = mulmod(numerator,ECCMath_noconflict.invmod(denominator, prime),prime);

      // val * (numerator / denominator) i.e. f(x_{j}) * (x_{m} / x_{m} - x_{j})
      if(formula == 0) {
        accum = Secp256k1_noconflict._mul(ndd, points[formula].points);
      } else {
        // Accumlate all values so far
        accum = Secp256k1_noconflict._add(accum,Secp256k1_noconflict._mul(ndd, points[formula].points));
      }
    }

    ECCMath_noconflict.toZ1(accum, pp);
    return accum;
  }

  // Prove equality of two discrete logs g^{a}, h^{a}. 'w' is the random factor. returns z;
  function createDiscreteLogEqualityGivenE(uint w, uint a, uint[2] gen1, uint[2] gen2, uint e) returns (uint[2] a1, uint[2] a2, uint z) {
    uint[3] memory j_a1 = Secp256k1_noconflict._mul(w,gen1);
    uint[3] memory j_a2 = Secp256k1_noconflict._mul(w,gen2);

    ECCMath_noconflict.toZ1(j_a1, pp);
    ECCMath_noconflict.toZ1(j_a2, pp);
    ...
    z = submod(w,mulmod(a,e,nn));
  }

  // Prove equality of two discrete logs g^{a}, h^{a}. 'w' is the random factor. returns z;
  function createDiscreteLogEqualityWithoutEZ(uint w, uint[2] gen1, uint[2] gen2) returns (uint[2] a1, uint[2] a2) {
    uint[3] memory j_a1 = Secp256k1_noconflict._mul(w,gen1);
    uint[3] memory j_a2 = Secp256k1_noconflict._mul(w,gen2);
  ...
  // Easier to store commitments in contract
  // And then perform the validation below.
  // Count from 0+ for commitment indexes
  mapping(uint => uint[2]) public commitments;
  uint n = 0;

  function storeCommitmentTESTING(uint id, uint[2] commitment) {
    commitments[id] = commitment;
    n = n + 1;
  }

  // Check all commitments using code-words.
  // r = random number for verification
  // n = total number of participants / commitments
  // t = threshold for shamir secret
  function validateCommitments(uint r, uint t) returns (bool) {

    uint[3] memory res;

    // We need to compute the dual-code
    // ** TODO: How do we sample random elements? **
    uint[] memory coef = new uint[](n-t-1); // test polynomial
    for(uint i=0; i<coef.length; i++) {
      coef[i] = addmod(uint(sha256(i, r)),1,nn); // Picking random elements in Fq
    }
    ...
    for(i = 1; i <=n; i++) {
      ...
      for(uint j=1; j<=n; j++) {
        ...
        if(i != j) {
          // We need to sum all 1 / i - j
          // So we do this as we go along
          if(v == 1) {
            v = mulmod(1,ECCMath_noconflict.invmod(submod(i,j,nn), nn),nn);
          } else {
            v = mulmod(v,mulmod(1,ECCMath_noconflict.invmod(submod(i,j,nn), nn),nn),nn);
          }
        }
      }

      // We want to create a code so (v, code) = (x,y)
      uint accum = coef[0];

      // Go through each co-efficient i.e. c + c_{1}x + c_{2}x^{2} + ... + c_{t-1}x^{t-1}
      for(j = 1; j < coef.length; j++) {

        // We do j-1 because we start counting from zero in coef
        uint coef_xexp = mulmod(coef[j-1],ECCMath_noconflict.expmod(i, j, nn),nn);
        accum = addmod(accum, coef_xexp, nn);
      }

      uint code = mulmod(v, accum, nn);
      uint[2] commitment = commitments[i-1];

      //TODO: Verify this is a real point on the curve
      if(i == 1) {
        res = Secp256k1_noconflict._mul(code, commitment);
      } else {
        res = Secp256k1_noconflict._add(res, Secp256k1_noconflict._mul(code, commitment));
      }
    }

    // Verification was performed during the distribution stage.
    if(res[0] == 0 && res[1] == 0) {
      return true;
    } else {
      return false;
  ...
  // Prove equality of two discrete logs g^{a}, h^{a}. 'w' is the random factor. returns z;
  function createDiscreteLogEquality(uint w, uint a, uint[2] gen1, uint[2] gen2, uint[2] c1, uint[2] c2) returns (uint[2] a1, uint[2] a2, uint z) {
    uint[3] memory j_a1 = Secp256k1_noconflict._mul(w,gen1);
    uint[3] memory j_a2 = Secp256k1_noconflict._mul(w,gen2);
    ...
    uint e = uint(sha256(msg.sender,a1,a2,gen1,gen2,c1,c2));

    z = submod(w,mulmod(a,e,nn));
  }

  // Verify equality proof for two discrete logs g^{a}, h^{a}.
  function verifyDiscreteLogEquality(uint z, uint[2] gen1, uint[2] gen2, uint[2] c1, uint[2] c2, uint[2] a1, uint[2] a2) returns (bool) {
    uint e = uint(sha256(msg.sender,a1,a2,gen1,gen2,c1,c2));

    // We compute gen1^{z}c1^{e}
    uint[3] memory temp = Secp256k1_noconflict._mul(z, gen1);
    uint[3] memory temp2 = Secp256k1_noconflict._mul(e, c1);
    uint[3] memory zGeX = Secp256k1_noconflict._add(temp, temp2);

    // Compare a1 = gen1^{z}c1^{e}
    ECCMath_noconflict.toZ1(zGeX, pp);
    if(a1[0] != zGeX[0] || a1[1] != zGeX[1]) {
      return false;
    }

    // We compute gen2^{z}c2^{e}
    temp = Secp256k1_noconflict._mul(z, gen2);
    temp2 = Secp256k1_noconflict._mul(e, c2);
    uint[3] memory zHeY = Secp256k1_noconflict._add(temp,temp2);

    // Compare a2 = gen2^{z}c2^{e}
    ECCMath_noconflict.toZ1(zHeY, pp);
    if(a2[0] != zHeY[0] || a2[1] != zHeY[1]) {
      return false;
    }
  ...
  // Compute Hash chain
  function hashchain(uint preimage, uint index) returns (uint) {
    uint h = uint(sha3(preimage));
  ...
  function createShares(uint[] coef, uint available) returns (uint[6] shares){
    uint prime = nn;

    // We want to create up to "available" shares.
    for(uint x=1; x<=available; x++) {

      uint accum = coef[0];

      // Go through each co-efficient i.e. c + c_{1}x + c_{2}x^{2} + ... + c_{t-1}x^{t-1}
      for(uint exp = 1; exp < coef.length; exp++) {
        uint xexp = ECCMath_noconflict.expmod(x, exp, prime);
        uint coef_xexp = mulmod(coef[exp],xexp,prime);
        accum = addmod(accum,coef_xexp,prime);
      }
