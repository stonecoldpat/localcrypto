...
contract SecondAccount {
  ...
  function equalityProof() returns (bool) {
    ...
    if(!con.verifyEqualityProof(n, c1,c2,t)) {
      return false;
    }
...
contract TestLocalCrypto2 {
  ...
  function testFirst() {
    con = LocalCrypto(DeployedAddresses.LocalCrypto());
  ...
  function testEqualityCommitmentProofs() {
    uint t = 123;
    Assert.equal(A.equalityProof(), true, "Equality Proof did not pass!");
    Assert.equal(t, 1, "Should Fail");
  }
  ...
  function testInequalityCommitmentProofs() {
    uint t = 123;
    Assert.equal(A.inequalityProof(), true, "Inequality Proof did not pass!");
    Assert.equal(t, 1, "Should Fail");
  }
...
/*
  ...
  function testSingleZKP() {
    bool[3] memory res = verifySingleZKP();
    uint t = 123;
    Assert.equal(res[0],true,"single zkp were false, not true! 0");
    Assert.equal(res[1],true,"single zkp were false, not true! 1");
    Assert.equal(res[2],true,"single zkp were false, not true! 2");
    Assert.equal(t, 1, "Should Fail");
  }
  ...
  function testOrZKP() {
    ...
    uint t = 1203;
    Assert.equal(res[0],true,"OR zkp were false, not true! 0");
    /*Assert.equal(res[1],true,"OR zkp were false, not true! 1");
    Assert.equal(res[2],true,"OR zkp were false, not true! 2");*/
    Assert.equal(t, 1, "Should Fail");
  }

  // Not all voters have cast their vote.. should throw
  function testCannotSubmitFakeZKP() {
    ...
    (res,nums) = D.singleZKP(v);

    Assert.equal(res, false, "Fake ZKP submitted");
    Assert.equal(v, 1, "Should Fail");
  }

  // Not all voters have cast their vote.. should throw
  function testCannotFakeVote() {
    uint x = 110792359988221257522464744073694181557998811287873941943642234039631667801743;
    uint[2] memory xG = [50011181273477635355105934748199911221235256089199741271573814847024879061899, 71802419974013591686591529237219896883303932349628173412957707346469215125624];
    uint[2] memory yG = [98038005178408974007512590727651089955354106077095278304532603697039577112780,1801119347122147381158502909947365828020117721497557484744596940174906898953];
    ...
    Assert.equal(res, false, "Wrong private key should have been used");
    Assert.equal(w, 1, "Should Fail");
  }
}
